...
const stats = [
  {
    ...
    icon: (
      <svg aria-hidden="true" fill="none" height="22" viewBox="0 0 24 24" width="22">
        ...
        <path d="M8 19V12" stroke="currentColor" strokeLinecap="round" strokeWidth="1.8" />
        <path d="M12 19V8" stroke="currentColor" strokeLinecap="round" strokeWidth="1.8" />
        <path d="M16 19V14" stroke="currentColor" strokeLinecap="round" strokeWidth="1.8" />
        <path d="M20 19V10" stroke="currentColor" strokeLinecap="round" strokeWidth="1.8" />
      </svg>
    ),
  },
  {
    ...
    icon: (
      <svg aria-hidden="true" fill="none" height="22" viewBox="0 0 24 24" width="22">
        <path d="M5 12.5 10 17 19 7" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
      </svg>
    ),
  },
  {
    ...
    icon: (
      <svg aria-hidden="true" fill="none" height="22" viewBox="0 0 24 24" width="22">
        <path d="M8 9h8M8 15h8M10 3h4" stroke="currentColor" strokeLinecap="round" strokeWidth="1.8" />
        <path d="M6 3h12v18H6z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" />
      </svg>
    ),
  },
];
...
const DashboardPage = () => {
  ...
  const logoutMutation = useMutation({
    mutationFn: () => handleGlobalPostRequest<{ success: boolean; message: string }, Record<string, never>>({
      url: "/api/auth/logout",
      data: {},
    }),
    onSuccess: async () => {
          ...
          <Dropdown
            ...
            items={[
              {
                ...
                icon: (
                  <span className="font-display text-lg uppercase text-primary-500">A</span>
                ),
              },
              {
                label: "Settings",
                ...
                icon: (
                  <svg aria-hidden="true" fill="none" height="18" viewBox="0 0 24 24" width="18">
                    ...
                    <path d="M19.4 15a7.8 7.8 0 0 0 .1-.95c0-.32-.03-.64-.1-.95l2-1.56-1.9-3.3-2.42.76a7.8 7.8 0 0 0-1.64-.95L13 5h-2l-.44 2.05c-.58.2-1.13.5-1.64.95l-2.42-.76-1.9 3.3 2 1.56c-.06.31-.1.63-.1.95 0 .32.04.64.1.95l-2 1.56 1.9 3.3 2.42-.76c.51.45 1.06.75 1.64.95L11 19h2l.44-2.05c.58-.2 1.13-.5 1.64-.95l2.42.76 1.9-3.3-2-1.56Z" stroke="currentColor" strokeLinejoin="round" strokeWidth="1.4" />
                  </svg>
                ),
              },
              {
                ...
                icon: (
                  <svg aria-hidden="true" fill="none" height="18" viewBox="0 0 24 24" width="18">
                    <path d="M10 17 15 12 10 7" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" />
                    <path d="M15 12H4" stroke="currentColor" strokeLinecap="round" strokeWidth="1.8" />
                    <path d="M12 4h5a3 3 0 0 1 3 3v10a3 3 0 0 1-3 3h-5" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" />
                  </svg>
                ),
                tone: "danger",
            ...
            trigger={
              <>
                ...
                <svg aria-hidden="true" fill="none" height="16" viewBox="0 0 24 24" width="16" className="text-muted transition group-hover:text-ink">
      ...
      <main className="relative mx-auto max-w-[1440px] px-4 pb-12 pt-6 sm:px-6 sm:pt-7">
          ...
          {[
            ...
            >
              <span className="text-base">
                {item.label === "My Quizzes" ? "▥" : item.label === "Host a Room" ? "▶" : "◷"}
              </span>
          ...
          {stats.map((stat, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`flex h-11 w-11 items-center justify-center rounded-lg ${index === 1 ? "bg-primary-500 text-black" : "bg-white/5 text-muted"}`}>
                  {stat.icon}
                </div>
                <div>
                  <div className={`font-display text-3xl uppercase tracking-[-0.03em] ${index === 1 ? "text-primary-500" : "text-ink"}`}>
                    {stat.value}
                  </div>
        ...
        <section className="mt-7 flex flex-wrap gap-2">
          {tabs.map((tab) => (
            ...
            >
              <span>{tab.label}</span>
              <span className={tab.key === "all" ? "text-ink/70" : "text-muted"}>{tab.count}</span>
            </button>
          ))}
        </section>
        ...
        <section className="mt-6 grid gap-4 xl:grid-cols-3">
          {quizCards.map((quiz) => (
            <article
              ...
              key={quiz.id}
            >
              <div className={`rounded-lg border border-white/5 bg-gradient-to-br p-4 ${quiz.accent}`}>
                <div className="flex items-start justify-between gap-4">
                  ...
                  <button className="rounded-full border border-line bg-black/30 px-2 py-1.5 text-muted transition hover:border-primary-700 hover:text-primary-500" type="button">
                    ⋯
                  </button>
                </div>
                ...
                <div className="mt-4 h-1 overflow-hidden rounded-full bg-white/5">
                  <div
                    className={`h-full rounded-full ${
                      quiz.status === "published" ? "bg-primary-500" : "bg-amber-500"
                    }`}
                    style={{ width: quiz.status === "published" ? "100%" : "58%" }}
                  />
                </div>
              </div>
                ...
                <div className="mt-4 flex items-center justify-between border-t border-line pt-4">
                  ...
                  <button className="rounded-lg border border-line px-3 py-1.5 font-display text-[11px] uppercase tracking-[0.14em] text-muted transition hover:border-primary-700 hover:text-ink" type="button">
                    Edit
                  </button>
                  <button className="rounded-lg border border-line px-3 py-1.5 font-display text-[11px] uppercase tracking-[0.14em] text-muted transition hover:border-primary-700 hover:text-ink" type="button">
                    Host
                ...
                <button className="rounded-lg border border-line px-3 py-1.5 text-sm text-muted transition hover:border-danger hover:text-danger" type="button">
                  🗑
                </button>
              </div>
            </article>
